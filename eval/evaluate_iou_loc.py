...
def lerf_localization(sem_map, image, clip_model, image_name, img_ann):
    ...
    for k in range(len(positives)):
        ...
        # Find the maximum point in the smoothed activated value map
        scale = 30
        kernel = np.ones((scale,scale)) / (scale**2)
        ...
        # Add the averaged result with the original result to suppress noise and keep the activated boundary clear
...
if __name__ == "__main__":
    ...
    feat_dir = [os.path.join(args.feat_dir, dataset_name+f"_{i}", "language/train/ours_30000/renders_npy") for i in range(1,4)]
