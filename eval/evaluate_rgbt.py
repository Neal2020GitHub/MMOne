...
def eval_gt_lerfdata(json_folder: Union[str, Path] = None, ouput_path: Path = None) -> Dict:
    ...
    """
    gt_json_paths = sorted(glob.glob(os.path.join(str(json_folder), '*mask.json')))
...
def activate_stream(sem_map, 
    ...
    for k in range(n_prompt):
        ...
        for i in range(n_head):
            # Find the maximum point in the activated value map after filtering
            scale = 30
            ...
            # Consider the activated value less than 0.5 as background
...
def evaluate(feat_dir, output_path, ae_ckpt_path, json_folder, mask_thresh, encoder_hidden_dims, decoder_hidden_dims, logger):
    ...
    for i in range(len(feat_dir)):
        # Since --eval splits the renders into train and test, the eval indices do not match, so we need to restore them
        train_feature_path = feat_dir[i]
        test_feature_path = train_feature_path.replace('train', 'test')
        train_feat_paths_lvl = sorted(glob.glob(os.path.join(train_feature_path, '*.npy')),
                                    key=lambda file_name: int(os.path.basename(file_name).split(".npy")[0]))
        test_feat_paths_lvl = sorted(glob.glob(os.path.join(test_feature_path, '*.npy')),
                                    key=lambda file_name: int(os.path.basename(file_name).split(".npy")[0]))
        for j, idx in enumerate(eval_index_list):
            if idx % 8 == 0:  # test
                compressed_sem_feats[i][j] = np.load(test_feat_paths_lvl[idx//8])
            else:
                compressed_sem_feats[i][j] = np.load(train_feat_paths_lvl[idx-(idx//8+1)])
    ...
    for j, idx in enumerate(tqdm(eval_index_list)):
        ...
        with torch.no_grad():
            ...
            restored_feat = restored_feat.view(lvl, h, w, -1)  # 1x480x640x512
...
if __name__ == "__main__":
    ...
    parser = ArgumentParser(description="evalute RGBT-Scenes")
    ...
    feat_dir = [os.path.join(args.feat_dir, "language/train/ours_30000/renders_npy")]
